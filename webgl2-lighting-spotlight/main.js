...
function drawScene(gl, program, vao, uniforms, data) {
    ...
    {
        let lmat = m4.lookAt(data.lightPosition, target)
        lmat = m4.multiply(m4.xRotation(data.lightRotation[0]), lmat)
        lmat = m4.multiply(m4.yRotation(data.lightRotation[1]), lmat)

        data.lightDirection = [-lmat[8], -lmat[9], -lmat[10]]
    }

    gl.uniform3fv(uniforms.u_lightDirection, data.lightDirection)

    gl.uniform1f(uniforms.u_limit, Math.cos(data.limit))
    ...
    gl.uniform3fv(uniforms.u_lightWorldPosition, data.lightPosition)
...
async function main() {
    ...
    let data = {
        ...
        shininess: 150,
        limit: webglUtils.degreesToRadians(20),
        lightRotation: [0, 0],
    ...
    const angle_converter = dataName => value => {
        data[dataName] = webglUtils.degreesToRadians(Number(value))
        redraw()
    }

    const number_converter = dataName => index => value => {
        data[dataName][index] = Number(value)
        redraw()
    }

    webglUtils.setupSliders({
        "Camera angle: ": {minValue: -360, maxValue: 360, currentValue: data.cameraAngleRadians, stepValue: 1, handle: angle_converter('cameraAngleRadians')},
        "Light rotation x: ": {
            minValue: -1,
            maxValue: 1,
            stepValue: 0.01,
            currentValue: data.lightRotation[0],
            handle: number_converter('lightRotation')(0)
        },
        "Light rotation y: ": {
            minValue: -1,
            maxValue: 1,
            stepValue: 0.01,
            currentValue: data.lightRotation[1],
            handle: number_converter('lightRotation')(1)
        },
        "Limit: ": {
            minValue: 0,
            maxValue: 180,
